...
stat_rows = []
for label, (field, fmt) in METRICS.items():
    stat_rows.append({
        "Metric": label,
        "Mean":   f"{filtered_df[field].mean():{fmt}}",
        "Min":    f"{filtered_df[field].min():{fmt}}",
        "Max":    f"{filtered_df[field].max():{fmt}}",
    })

st.dataframe(
    pd.DataFrame(stat_rows).set_index("Metric"),
